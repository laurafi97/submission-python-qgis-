...
# Determine path to working directory ("project")
...
def main():
    ...
    fn = my_folder / input_subfolder / fn_land_parcel
    land_parcel = my_add_vector_layer_from_shapefile(fn, 'land_parcel')
    
    fn = my_folder / input_subfolder / fn_buildings
    buildings = my_add_vector_layer_from_shapefile(fn, 'buildings')
    
    fn = my_folder / input_subfolder / fn_area_of_interest
    ...
    crop_land_parcel = my_processing_run_vector_clip("native:clip", land_parcel, area_of_interest, dict_params, 'land_parcel_crop')
    
    dict_params = {'INPUT': buildings, 'OVERLAY': area_of_interest, 'OUTPUT': 'TEMPORARY_OUTPUT'}
    crop_buildings = my_processing_run_vector_clip("native:clip", buildings, area_of_interest, dict_params, 'buildings_crop')
    ...
    joined_layer = my_join_attributes_by_location(crop_buildings, crop_land_parcel, join_fields, output_layer_name)
   ...
   # Aggregate the 'Buildings_Area' attribute by parcel ID (idflurst)
    ...
    aggregate_result = my_aggregate_function(joined_layer, '"idflurst"', [
        {'aggregate': 'first_value', 'delimiter': ',', 'input': '"idflurst"', 'length': 16, 'name': 'idflurst', 'precision': 0, 'sub_type': 0, 'type': 10, 'type_name': 'text'},
        {'aggregate': 'sum', 'delimiter': ',', 'input': '"Buildings_Area"', 'length': 10, 'name': 'Buildings_Area', 'precision': 0, 'sub_type': 0, 'type': 6, 'type_name': 'double precision'}
    ...
    join_result = my_join_attributes_by_field_value(crop_land_parcel, aggregate_result, 'idflurst', 'idflurst', ['Buildings_Area'], output_layer_name=joined_by_field_value)
    ...
    fn_land_parcel_buildings_area = my_folder / output_subfolder / 'land_parcel_buildings_area.shp'
